...
sub save_product_info {
	...
	my $insertProductInfo = $dbh->prepare_cached('INSERT INTO product VALUES (?,?,?,?,?,?,?,?,?)');
	
	die "Couldn't prepare queries" unless defined $insertProductInfo;
	
	$insertProductInfo->execute($sku, $category, $vendor, $model, $description, $features, $cost, $retail, $newFilename) or die "Failed to insert product data";
